...
class UserController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $crud = new CrudController(new RecordModel(), $request, $this->selectFields , [
            ...
            'pdf' => function($record){
                $record->pdf = ( strlen( $record->pdf ) > 0 && \Storage::disk('user')->exists( $record->pdf ) )
                ? true
                : false ;
                return $record->pdf ;
            }
    ...
    public function updatePasscode(Request $request){
        ...
        if( isset( $request->passcode ) || ( isset( $request->passcode ) && intval( $request->passcode ) < 0 ) ){
            return response()->json([
                'ok' => false ,
                'message' => 'សូមបញ្ចូលលេខកូដឆ្លងកាត់អោយបានត្រឹមត្រូវ។'
            ],403);
        }

        $record->passcode = str_pad( $request->passcode  , 6, "0", STR_PAD_LEFT) ;
        $record->update();
    ...
    public function uploadPicture(Request $request){
        ...
        if( $user ){
            ...
            if( ( $record = RecordModel::find($request->id) ) !== null ){
                $uniqeName = Storage::disk('public')->putFile( 'users/'.$user->id , new File( $_FILES['files']['tmp_name'] ) );
    ...
    public function uploadPdf(Request $request){
        ...
        if( $user ){
            ...
            $mbFilesize = round( $kbFilesize / 1024 , 4 );
            if( ( $record = RecordModel::find($request->id) ) !== null ){
                $uniqeName = Storage::disk('user')->putFile( '' , new File( $_FILES['files']['tmp_name'] ) );
                $record->pdf = $uniqeName ;
                $record->save();
                if( Storage::disk('user')->exists( $record->pdf ) ){
    ...
    public function pdf(Request $request)
    {
        ...
        if($record) {
            $pathPdf = storage_path('data') . '/users/' . $record->pdf ;
