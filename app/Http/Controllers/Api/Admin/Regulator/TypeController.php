...
class TypeController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $crud = new CrudController(new RecordModel(), $request, $this->fields , [
            ...
            'pdf' => function($record){
                $record->pdf = ( strlen( $record->pdf ) > 0 && \Storage::disk('type')->exists( $record->pdf ) )
                ? true
                : false ;
                return $record->pdf ;
            }
    ...
    public function compact(Request $request)
    {
        ...
        //         'ok' => false ,
        //         'message' => 'ប្រភេទឯកសារនេះមិនមានឡើយ។'
        //     ],422);
        // }
    ...
    public function addChild(Request $request){
        ...
        if( $parent == null || $child == null ){
            return response()->json([
                'ok' => false ,
                'message' => "សូមជ្រើសរើស ប្រភេទឯកសារ "
            ],350);
        }
        $child->pid = $parent->id ;
        $child->save();
    ...
    public function uploadPicture(Request $request){
        ...
        if( $user ){
            ...
            if( ( $record = RecordModel::find($request->id) ) !== null ){
                $uniqeName = Storage::disk('public')->putFile( 'types/'.$user->id , new File( $_FILES['files']['tmp_name'] ) );
                ...
                if( $record->image != null && strlen( $record->image ) > 0 && Storage::disk('public')->exists( $record->image ) ){
                    ...
                    return response([
                        'record' => $record ,
                        'ok' => false ,
                        'message' => 'មិនមានប្រភេទឯកសារដែលស្វែងរកឡើយ។'
                    ],403);
                }
            }else{
                return response([
                    'ok' => false ,
                    'message' => 'សូមបញ្ជាក់អំពីលេខសម្គាល់របស់ប្រភេទឯកសារ។'
                ],403);
            }
        }else{
            return response([
    ...
    public function uploadPdf(Request $request){
        ...
        if( $user ){
            ...
            $mbFilesize = round( $kbFilesize / 1024 , 4 );
            if( ( $record = RecordModel::find($request->id) ) !== null ){
                $uniqeName = Storage::disk('type')->putFile( '' , new File( $_FILES['files']['tmp_name'] ) );
                $record->pdf = $uniqeName ;
                $record->save();
                if( Storage::disk('type')->exists( $record->pdf ) ){
    ...
    public function pdf(Request $request)
    {
        ...
        if($record) {
            $pathPdf = storage_path('data') . '/types/' . $record->pdf ;
