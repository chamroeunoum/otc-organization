...
{
    private $model = null ;
    private $fields = [ 'id','name','desp' , 'pid' , 'model' , 'pdf' , 'image', 'tpid' , 'code' , 'record_index' , 'active' ] ;
    ...
    public function index(Request $request){
        ...
        $parent = isset( $request->id ) && intval( $request->id ) > 0? RecordModel::find( intval( $request->id ) ) : null ; // 163 គឺ រាជរដ្ឋាភិបាល
        ...
        $crud = new CrudController(new RecordModel(), $request, $this->fields , [
            ...
            'pdf' => function($record){
                $record->pdf = ( strlen( $record->pdf ) > 0 && \Storage::disk('organization')->exists( $record->pdf ) )
                ? true
                : false ;
                return $record->pdf ;
            }
    ...
    public function compact(Request $request)
    {
        ...
        $parentNode = isset( $request->id ) && intval( $request->id ) > 0? RecordModel::find( intval( $request->id ) ) : null ; // 163 គឺ រាជរដ្ឋាភិបាល
    ...
    public function update(Request $request){
        ...
        $updateData = [
            ...
            // 'tpid' => $parentNode == null ? null : ( $parentNode->tpid != null && $parentNode->tpid != "" ? $parentNode->tpid .':'. $parentNode->id : $parentNode->id ) ,
        ];
    ...
    public function uploadPicture(Request $request){
        ...
        if( $user ){
            ...
            if( ( $record = RecordModel::find($request->id) ) !== null ){
                $uniqeName = Storage::disk('public')->putFile( 'organizations/'.$user->id , new File( $_FILES['files']['tmp_name'] ) );
    ...
    public function uploadPdf(Request $request){
        ...
        if( $user ){
            ...
            $mbFilesize = round( $kbFilesize / 1024 , 4 );
            if( ( $record = RecordModel::find($request->id) ) !== null ){
                $uniqeName = Storage::disk('organization')->putFile( '' , new File( $_FILES['files']['tmp_name'] ) );
                $record->pdf = $uniqeName ;
                $record->save();
                if( Storage::disk('organization')->exists( $record->pdf ) ){
    ...
    public function pdf(Request $request)
    {
        ...
        if($record) {
            $pathPdf = storage_path('data') . '/organizations/' . $record->pdf ;
