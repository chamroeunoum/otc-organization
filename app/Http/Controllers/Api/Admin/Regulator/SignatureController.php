...
use App\Models\Regulator\Tag\Signature as RecordModel;
...
class SignatureController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $crud = new CrudController(new RecordModel(), $request, $this->fields , [
            ...
            'pdf' => function($record){
                $record->pdf = ( strlen( $record->pdf ) > 0 && \Storage::disk('signature')->exists( $record->pdf ) )
                ? true
                : false ;
                return $record->pdf ;
            }
    ...
    public function uploadPdf(Request $request){
        ...
        if( $user ){
            ...
            $mbFilesize = round( $kbFilesize / 1024 , 4 );
            if( ( $record = RecordModel::find($request->id) ) !== null ){
                $uniqeName = Storage::disk('signature')->putFile( '' , new File( $_FILES['files']['tmp_name'] ) );
                $record->pdf = $uniqeName ;
                $record->save();
                if( Storage::disk('signature')->exists( $record->pdf ) ){
    ...
    public function pdf(Request $request)
    {
        ...
        if($record) {
            $pathPdf = storage_path('data') . '/signatures/' . $record->pdf ;
